...
登録もログインも「① ブラウザが仲介 → ② サーバーが challenge 発行 → ③ サーバーが検証」の3手順。passkey 固有のコードは実質この6箇所だけ。

## 登録

**① ブラウザ: options をもらって Face ID → 結果を送る** — `src/routes/+page.svelte`
...
**② サーバー: challenge を発行して D1 に保存** — `src/routes/api/register/options/+server.ts`
...
**③ サーバー: 検証して公開鍵を保存** — `src/routes/api/register/verify/+server.ts`
...
**① ブラウザ: ユーザー名なしで開始** — `src/routes/+page.svelte`
...
**② サーバー: challenge 発行** — `src/routes/api/login/options/+server.ts`
...
**③ サーバー: credential ID から逆引き → 署名検証 → セッション発行** — `src/routes/api/login/verify/+server.ts`
...
```bash
pnpm install
pnpm wrangler d1 migrations apply pasu --local
pnpm dev        # ローカル起動
pnpm test:unit  # Vitest(ローカル D1)
pnpm test:e2e   # Playwright(仮想認証器)
```

main への push で CI がテスト → D1 マイグレーション → デプロイまで自動実行する。
